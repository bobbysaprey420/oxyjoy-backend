...
router.get('/create-order-table', (req, res) => {
    let sql = "CREATE TABLE orders(order_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, status VARCHAR(128) DEFAULT 'NOT CONFIRMED', insert_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES user(user_id))"
    mysqlConnection.query(sql, (err, result) => {
        if(err){
            console.log(err);
